        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case TWEET_ID:
                ...
                if (TextUtils.isEmpty(selection)) {
                    selection = DatabaseContract.Tweet.TWEET_ID +
                            " = " + uri.getLastPathSegment();
                } else {
                    selection = selection + " AND " + DatabaseContract.Tweet.TWEET_ID +
                            " = " + uri.getLastPathSegment();
                }
                cursor = mTweetDatabaseHelper.getReadableDatabase()
                        .query(tableName, projection, selection, selectionArgs,
            ...
            case USER_ID:
                tableName = DatabaseContract.User.TABLE_NAME;
                if (TextUtils.isEmpty(selection)) {
                    selection = DatabaseContract.User.USER_ID +
                            " = " + uri.getLastPathSegment();
                } else {
                    selection = selection + " AND " + DatabaseContract.User.USER_ID +
                            " = " + uri.getLastPathSegment();
                }
                cursor = mTweetDatabaseHelper.getReadableDatabase()
                        .query(tableName, projection, selection, selectionArgs,
